\section{Grafos Expansores}
\begin{frame}
  \begin{itemize}[<+-| alert@+>] 
    \item Existem muitas definições de grafos expansores
    \item Todas tem uma mesma ideia: grafo esparso e altamente conexo 
    \item Vamos ver uma das possíveis definições 
  \end{itemize}
\end{frame}

\subsection{Expansão}


\begin{frame}{Altamente conectado}
  Precisamos definir a ideia de um grafo altamente conexo:
  \onslide<2->{%
    \begin{center}
    \huge Que tal o número o número de corte??
    \end{center}
  }
\end{frame}

\begin{frame}
  \begin{example}
  Considere o seguinte grafo $(n \ge 100)$:
  \begin{center}
  \begin{tikzpicture}
      \node (KN) {
          \begin{tikzpicture}
            \node[draw, ellipse, minimum width=2cm, minimum height=5cm,
              label=center:$K_n$] (A) at (0,0) {};
            \node[draw, ellipse, minimum width=2cm, minimum height=5cm,
              label=center:$K_n$] (B) at (6,0) {};
            \draw (A.east) -- (B.west);
            \node at (3,0.8) {100 edges};
          \end{tikzpicture}
        };

        \onslide<2->{%
          \node[right=10pt] at (KN.east) {$\lim_{n\rightarrow
          +\infty}{\frac{\lambda(K_n)}{|K_n|}}$ = 0};
        };
    \end{tikzpicture}
  \end{center}
  \end{example}
  \end{frame}

\begin{frame}{Constante de Cheeger ou Razão de Expansão}
  Precisamos de outra forma de medir isso:  
  \onslide<2->{%
    \begin{definition}
      Seja $G = (V, E)$ um grafo. A constante de Cheeger de $G$, denotada por $h(G)$, é definida por
      \[
        h(G) = \min_{S\subsetneq V }\frac{|\partial S|}{\min(|S|, |V|\setminus|S|)}
      \]
    \end{definition}
  }
\end{frame}

\begin{frame}
  \begin{example}
    Considere o grafo completo $K_n$ e $S \subset V$ um subconjunto com m
    vertíces, temos que:

    \[
      |\partial S| = m(n-m)
    \]

    Então:

    \[
      \frac{|\partial S|}{\min(|S|,|V|\setminus|S|)} =\frac{m(n-m)}{\min(m, n-m)} = \max(m , n - m).
    \]

    O valor que minimiza a expressão é $m=\frac{n}2$, logo teremos que:

    \[
        h(G) = \frac{n}2
    \]

    \vspace{5pt}

  \end{example}
\end{frame}


\subsubsection{Expansão e lacuna espectral}
\subsubsection{}

\begin{frame}
     \begin{thebibliography}{99}
        \bibitem{article1}Author
        \emph{Title}
        journal
    \end{thebibliography}
\end{frame}

\begin{frame}
    \begin{center}
        {\Huge Thanks!}
    \end{center}
\end{frame}


